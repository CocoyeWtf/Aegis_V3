/* Ecran reception base — scanner etiquettes retour / Base reception scanner screen
   Flow : camera 1D → scan etiquette RET-xxx → POST /driver/base-receive/{code} → compteur
*/
...
export default function BaseReceptionScreen() {
  ...
  const [scans, setScans] = useState<BaseReceiveScan[]>([])
  ...
  useEffect(() => {
    api.get<BaseReceiveScan[]>('/driver/base-receives')
  ...
  if (!permission.granted) {
    return (
      <SafeAreaView style={styles.center}>
        <Text style={styles.text}>Acces camera requis pour scanner les retours</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Reception base</Text>
        <View style={{ width: 60 }} />
      </View>
      ...
      {/* Liste receptionnee / Received list */}
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <View style={styles.scanRow}>
              ...
              <View style={styles.scanRight}>
                <Text style={styles.scanStatus}>OK</Text>
                {item.received_at && (
                  <Text style={styles.scanTime}>
                    {new Date(item.received_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                  </Text>
                )}
              </View>
            </View>
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Aucune reception aujourd'hui</Text>
          }
        />
      )}
    </SafeAreaView>
