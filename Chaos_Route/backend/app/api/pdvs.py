...
async def list_pdvs(region_id: int | None = None, db: AsyncSession = Depends(get_db)):
...
async def get_pdv(pdv_id: int, db: AsyncSession = Depends(get_db)):
...
async def create_pdv(data: PDVCreate, db: AsyncSession = Depends(get_db)):
...
async def update_pdv(pdv_id: int, data: PDVUpdate, db: AsyncSession = Depends(get_db)):
    pdv = await db.get(PDV, pdv_id)
    if not pdv:
        raise HTTPException(status_code=404, detail="PDV not found")
...
async def delete_pdv(pdv_id: int, db: AsyncSession = Depends(get_db)):
    pdv = await db.get(PDV, pdv_id)
    if not pdv:
        raise HTTPException(status_code=404, detail="PDV not found")
    await db.delete(pdv)
