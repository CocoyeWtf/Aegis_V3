...
export default function ReceptionBooking() {
  ...
  const fetchPickups = useCallback(async () => {
    try {
      const [pRes, cRes] = await Promise.all([
        api.get('/reception-booking/pickups/', { params: { base_id: selectedBaseId || undefined, pickup_status: pickupFilter || undefined } }),
        api.get('/carriers/').catch(() => ({ data: [] })),
      ])
      setPickups(pRes.data)
      setCarriers(cRes.data)
    } catch { /* silent */ }
  }, [selectedBaseId, pickupFilter])
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {viewParam === 'transport' && (
        <div className="space-y-4">
          {/* Compteurs par statut pickup — cliquables comme filtres */}
                ...
                {pickups.map((p) => {
                  ...
                  <div key={p.id} className="px-4 py-3 flex items-center gap-3"
                    ...
                    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full text-white flex-shrink-0"
                      style={{ backgroundColor: PICKUP_STATUS_COLORS[ps] }}>
                      {PICKUP_STATUS_LABELS[ps]}
                    </span>
                    {/* Fournisseur */}
                    ...
                    <div className="text-right flex-shrink-0">
                      <div className="text-xs" style={{ color: 'var(--text-primary)' }}>
                        Enl: <strong>{p.pickup_date || '?'}</strong>
                      </div>
                      <div className="text-[10px]" style={{ color: 'var(--text-muted)' }}>
                        Recep: {p.booking_date}
                      </div>
                    </div>
                    {/* Palettes */}
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      ...
                      {nextStatus[ps] && (
                        <button onClick={(e) => { e.stopPropagation(); handlePickupStatus(p.id, nextStatus[ps]) }}
                          className="px-2 py-1 rounded text-[10px] font-medium text-white"
                          style={{ backgroundColor: PICKUP_STATUS_COLORS[nextStatus[ps]] }}>
                          {PICKUP_STATUS_LABELS[nextStatus[ps]]}
                        </button>
                      )}
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              ...
              {!assignInternal && (
                <div>
                  ...
                  <select value={assignCarrierId} onChange={(e) => setAssignCarrierId(Number(e.target.value) || '')}
                    className="w-full px-3 py-2 rounded-lg text-sm border"
                    style={{ backgroundColor: 'var(--bg-tertiary)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }}>
                    <option value="">-- Choisir --</option>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" step="0.01" min={0} value={assignPrice} onChange={(e) => setAssignPrice(e.target.value)}
                    className="w-full px-3 py-2 rounded-lg text-sm border"
                    style={{ backgroundColor: 'var(--bg-tertiary)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Ref transport</label>
                  <input type="text" value={assignRef} onChange={(e) => setAssignRef(e.target.value)}
                    className="w-full px-3 py-2 rounded-lg text-sm border"
                    style={{ backgroundColor: 'var(--bg-tertiary)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }} />
                </div>
              </div>
